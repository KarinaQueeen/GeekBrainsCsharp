...
*(<T> используется в методе для того чтобы он работал с любым типом данных)*
    ...
    {
        a = ...                                       // обязательное присваивание значения в теле метода 
    }   
...
*(Ключевое слово out)*
    ...
    void Metod (in int a) 
    {
                                                    // нельзя присвить значение в теле метода 
    } 
...
*(Рекурсия - вызов метода самого себя)*
    ...
    {}                            _
    double a = 5.3;                |  
    float b = (float) a;           |-  Metod ((float) a);  
    Metod (b);                    _|

    int a = 5;
    float b = 2.5f;                // можно использовать f или F
    float result = a + b; 

    int a = 5;
    float b = 2.5f;                
    int result = (int) (a + b);     // преведение дробного результата к целому

*(Преобразование и приведение примитивных типов: тип int неявно автоматически преобразовывается в float, double не преобразовывается автоматически в float. Это происходит из за того что у разных типов данных разный диапозон значений.)*

    double a = 5.2;
    bool b = Convert.ToBoolean (a);

*(Преобразование в bool необходимо детать через Convert.ToBoolean и значение будет a <= 0 : false; b > 0 : true)*

    int a  = int.MaxValue;
    a = a + 1;                         // переполнение через верхнюю границу

    int a  = int.MinValue;
    a = a - 1;                         // переполнение через нижнюю границу

*(Переполнение происходит когда данные выходят за диапозон возможных данных этого типа)*
...
*(Используется для отслеживания на переполнение)*

    unchecked ();

*(Используется для отмены отслеживания в конкретном месте на переполнение, если во всем проекте включено отслеживание)*

    int? a = null;
    Console.WriteLine (a == null);                                  // True
    Console.WriteLine (a.HasValue);                                 // False
    Console.WriteLine (a.Value);                                    // ошибка
    Console.WriteLine (a.GetValueOrDefault());                      // 0          возвращает дефолтное значение типа данных
    Console.WriteLine (a.GetValueOrDefault(3));                     // 3          присваивает значение вместо дефолтного
    Console.WriteLine (a ?? 5);                                     // 5          проверит переменную, если null присвоит значение

*(Nullable позволяет присвоить null значимым типам данных, если там нет данных)*
